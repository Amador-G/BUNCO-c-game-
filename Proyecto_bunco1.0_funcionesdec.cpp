   ...
   while(!elegir){
    if(_kbhit()){
       ...
       char tecla = _getch();
        ...
        if(tecla==ABAJO && y < 17) locate(x,y++);
        locate(x,y);printf("*");}
   }
}
...
void Opciones(int v,int Mganador[],int tam,char Jmaximo[],char Janterior[]){
    ...
    case 15:///puntahe maximo
         {
             if(Mganador[0]==0){
                locate(30,11);
                cout<<"Debe completar al menos una partida en modo 2 jugadores";
                locate(30,12);
...
cout<<"Si en la primera tanda de tiradas de una ronda el jugador 1 logra este puntaje sin ninguna tirada fallida,"<<endl;
cout<<"el segundo jugador puede realizar lanzamientos hasta cumplir con la condición de finalización de ronda o realizar una tirada fallida."<<endl<<endl<<endl;
system("pause");
}
...
void LanzamientoDeDadosManual(int v[],int tam){
      int j;
       // AnimacionDados(v,tam);
        cargarVector(v,tam);
        MostrarDados(v,tam);
...
int CalcularPuntajeTirada(int v[],int  tam,int ronda){
    ...
    MaxPuntaje=puntos[0];
    pos=maximoVector(puntos,5);//devuelve la posicion donde se encuentra el numero mas alto
    if(puntos[pos]!=0){
            locate(20,7);
            cout<<"SACASTE: ";
            switch(pos){
...
///----------------------------------------///
    ...
    while(RondaTerminada == false){
        ...
        UnJugador[5]++;//lanzaientos totales ++ (no se reinicia)
        ...
        else{LanzamientoDeDadosManual(dados,3);}
       ...
       PuntajeFinal=UnJugador[1]-(UnJugador[2]*2);//calcula el puntaje menos las fallidas
       FinDePartida1Jugador (NombreJugador1, UnJugador,6, PuntajeFinal);//muestra los resultados de la partida
}///FIN DE LA FUNCION
...
void FinDePartida1Jugador (char NombreJugador[], int UnJugador[], int tam, int PuntajeFinal){
    int i;
    for(i=0;i<2;i++){
    recuadro(32,1,56,26);
    recuadro(33,2,54,24);
...
int dados[3],Puntaje,PrimerJugador[6]={0},SegundoJugador[6]={0};
int i;
...
for(i=0;i<6;i++){
       ...
       PrimerJugador[5]=0;//ACUMULADOR PUNTAJE DE RONDAS.
       SegundoJugador[5]=0;//ACUMULADOR PUNTAJE DE RONDAS.
...
while(PrimerJugador[5]<21 && SegundoJugador[5]<21){
       PrimerJugador[3]=0;//PUNTAJE DE TURNO
       SegundoJugador[3]=0;//PUNTAJE DE TURNO
       TurnoJugador1=true;
       TurnoJugador2=true;
       ProximoTurno=false;///false = PROXIMO TURNO JUGADOR 2/ true = PROXIMO TURNO JUGADOR 1
...
if(Puntaje==0){PrimerJugador[2]+=1;
     ...
     PrimerJugador[5]+=Puntaje;}

if (PrimerJugador[3]>=21||Puntaje==0){TurnoJugador1=false;}
recuadro(2,15,22,2);
recuadro(2,18,22,2);
locate(3,16);
cout << "PUNTAJE DEL TURNO: "<< PrimerJugador[3];
locate(3,19);
cout<<"PUNTAJE DE RONDA: "<< PrimerJugador[5];

locate(1,30);
system("pause");
cls();
}///FIN  WHILE TURNO JUGADOR UNO
PrimerJugador[1]+=PrimerJugador[3];
if(PrimerJugador[5]>=21){TurnoJugador2=false;}
else{
///ENTRE TURNOS
...
recuadro(2,18,22,2);
locate(3,16);
cout << "PUNTAJE DEL TURNO: "<< SegundoJugador[3];
locate(3,19);
cout<<"PUNTAJE DE RONDA: "<< SegundoJugador[5];

rlutil::locate (1,30);
locate(1,30);
system("pause");
cls();}///FIN TURNO JUGADOR DOS (while)
SegundoJugador[1]+=SegundoJugador[3];

///ENTRE RONDAS
ProximoTurno=true;///PROXIMO TURNO JUGADOR 1
EntreRondas(Nombre_Primer_Jugador, PrimerJugador, Nombre_Segundo_Jugador, SegundoJugador,i,ProximoTurno);}///FIN WHILE TURNOS



}///FIN DE LAS RONDAS (for)
...
rlutil::locate(25,12);
	...
	rlutil::locate(25,14);
	cout << "PUNTAJE DE "<< Nombre_Primer_Jugador << " : " << PrimerJugador[5] << " PUNTOS";
    rlutil::locate(25,15);
	cout << "BUCOS : " << PrimerJugador[0];
	rlutil::locate(25,17);
	cout << "PUNTAJE DE "<< Nombre_Segundo_Jugador << " : "<<SegundoJugador[5]<< " PUNTOS";
	rlutil::locate(25,18);
